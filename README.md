...
- The results presented are from 4 different libraries with the following search
  ...
  |-------------|-----------------------|
  | IEEE explore |(((arm OR upper-limb) AND (human-like OR legible OR predictable) AND (motion OR movement OR trajectory)) AND (humanoid OR robot) AND (planning OR generation) )
E os seguintes filtros aplicados:
-Conferences
-Journals
- 2006-2025|
